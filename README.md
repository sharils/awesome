...
1. Triple tap on the trackpad and start dragging to select line by line
1. Hold `⌘` and `⇧` then double tap to select a file path in the Terminal app
1. Hold `⌘` then tap to open a URL in the Terminal app; also hold `fn` when mouse reporting is turned on.
...
1. ``" #$%&+,/:;<=>?@[\]^`{|}`` in a password have to be URI encoded when used in a connection string.
